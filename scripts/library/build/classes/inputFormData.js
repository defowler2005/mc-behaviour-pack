...
export class inputFormData {
     ...
     * @param {Array<[string, Array<string>, number]>} info.dropdown - The dropdowns to add to the form, each represented by a name, an array of options, and a default option index.
     ...
     * @param {Function(ModalFormResponse): void} callback - The callback function to execute when the form is submitted.
     */
    create(info, callback) {
        try {
            ...
            if (info?.slider?.length >= 1) {
                for (const [name, input0, input1, input2] of info.slider) {
                    this.form.slider(name, input0, input1, input2);
                }
            }

            if (info?.toggle?.length >= 1) {
                for (const [name, input0] of info.toggle) {
                    this.form.toggle(name, input0);
                }
            }

            if (info?.dropdown?.length >= 1) {
                for (const [name, options, defaultOpt] of info.dropdown) {
                    this.form.dropdown(name, options, defaultOpt);
                }
            }

            if (info?.textField?.length >= 1) {
                for (const [name, hoverText, defaultText] of info.textField) {
                    this.form.textField(name, hoverText, defaultText);
                }
            }
            ...
            console.error(`An error occurred while creating the inputFormData form ${info.title}: ${error}\n${error.stack}`);
        }
    }
};
